...
def _agent_color(name: str, agent_type: str) -> str:
    ...
    t = agent_type.lower()
    for key in (n, t):
        if key in ('maps', 'human', 'hermes', 'wizard', 'cassette', 'claude', 'codex', 'test_agent'):
            return key
    return 'default_fg'
...
class ChatTUI:
    ...
    def _build_ui(self):
        self._header_text = urwid.Text(
            [('header', f' POLYCULE '), ' ', ('header', f'room: {self.room}'),
             ' ', ('header', f'[{self.name}]'), '  connecting…'],
            align='left',
        )
        self.header = urwid.AttrMap(self._header_text, 'header')

        self.walker = urwid.SimpleFocusListWalker([
            SystemMessage('connecting to hub…')
        ])
    ...
    async def connect(self):
        try:
            ...
            if response.get('action') == 'awaiting_room':
                cmd = {'type': 'command', 'command': 'create_room', 'room_name': self.room}
            ...
            if response.get('type') in ('room_created', 'room_state'):
                ...
                self.add_system(f'connected · room: {self.room} · {agent_count} agent(s)')
                self._update_header()
    ...
    async def _handle_incoming(self, msg: dict):
        ...
        if t == 'message':
            ...
            if action == 'agent_joined':
                ...
                self.add_system(f'auto-approve: {status}', 'granted' if msg.get('value') else 'denied')
            ...
            self.add_system(
                ...
                f"(by {msg.get('approved_by')})", 'granted'
            )

        elif t == 'approval_denied':
            self.add_system(
                f"✗ denied [{msg.get('request_id')}]", 'denied'
            )
    ...
    async def _handle_user_input(self, text: str):
        ...
        if parts[0].lower() == 'approve' and len(parts) == 2:
            await self._send({'type': 'command', 'command': 'approve', 'request_id': parts[1]})
            return
        if parts[0].lower() == 'deny' and len(parts) == 2:
            await self._send({'type': 'command', 'command': 'deny', 'request_id': parts[1]})
            return
    ...
    async def _handle_slash(self, cmd: str):
        ...
        if verb == 'approve' and len(parts) >= 2:
            ...
            await self._send({'type': 'command', 'command': 'set_auto_approve', 'value': val})
        elif verb == 'rooms':
            await self._send({'type': 'request', 'request': 'rooms'})
        elif verb == 'join' and len(parts) >= 2:
            await self._send({'type': 'command', 'command': 'join_room', 'room_id': parts[1]})
